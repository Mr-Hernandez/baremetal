...
__attribute__ ((naked)) void switch_sp_to_psp(void)
{
	...
	__asm volatile ("PUSH {LR}"); // preserve LR which connect bak to main()
...
void schedule(void)
{
	uint32_t *pICSR = (uint32_t*)0x0E000ED04;
	*pICSR |= (1 << 28); // pendsv exception
...
void task_delay(uint32_t tick_count)
{
	// disable interrupt to avoid global being used by intterupt and thread mode
	INTERRUPT_DISABLE();
	if(current_task)
	{
...
__attribute__((naked)) void PendSV_Handler(void)
{
	...
	// extra step, save contents of lr register
	__asm volatile ("PUSH {LR}");
	// 3. Save the current vlaue of PSP
